...
class DetectionHead_v4(nn.Module):
    def __init__(self, model_config: ModelConfig | None = None):
        super().__init__()
        if model_config is None:
            self.model_config = ModelConfig()
        ...
        # 1. Bounding Box Head (Outputs 4 coordinates)
        self.box_head = nn.ModuleList([
            nn.Sequential(
        ...
        # 2. Classification Head (Outputs num_classes)
        self.cls_head = nn.ModuleList([
            nn.Sequential(
        ...
        # 3. NEW: Centerness Head (Outputs 1 score)
    ...
    def _initialize_biases(self):
        ...
        # We also initialize the centerness bias similarly to avoid early instability
        for m in self.center_head:
            last_conv = m[-1]
            nn.init.constant_(last_conv.bias, -math.log((1 - prior) / prior))
    ...
    def forward(self, x: list[torch.Tensor]):
        ...
        # NEW: Forward pass for centerness
        centerness = torch.cat([head(x[i]).view(x[i].shape[0], 1, -1) for i, head in enumerate(self.center_head)], dim=-1)
        
        return bboxes, bbox_cls, centerness
...
class HandGestureModel_v4(nn.Module):
    def __init__(self, model_config: ModelConfig | None = None):
        super().__init__()
        if model_config is None:
            self.model_config = ModelConfig()
        ...
        self.detection_head = DetectionHead_v4(model_config=self.model_config)
    ...
    def forward(self, x):
        ...
        bboxes, bbox_cls, centerness = self.detection_head(features)  
        seg_map = self.segmentation_head(features)            
        
        return cls_logits, bboxes, bbox_cls, centerness, seg_map



if __name__ == "__main__":

    print("\nTesting HandGestureModel_v4:")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_v4 = HandGestureModel_v4().to(device)
    model_v4.eval()
    torchinfo.summary(model_v4, input_size=(1, 3, 640, 480), device=str(device))

    dummy_x = torch.randn(1, 3, 640, 480, device=device)
    with torch.no_grad():
        cls_logits, bboxes, bbox_cls, centerness, seg_map = model_v4(dummy_x)
    print(f"Classifier logits shape: {cls_logits.shape}")
    print(f"Bounding box predictions shape: {bboxes.shape}")
    print(f"Bounding box class scores shape: {bbox_cls.shape}")
    print(f"Centerness scores shape: {centerness.shape}")
