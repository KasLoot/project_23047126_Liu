...
class SegAugment_v2:
    ...
    def __call__(self, image, mask, bbox, like_or_dislike):
        ...
        angle_limit = 0.0 if like_or_dislike else 20.0
...
class HandGestureDataset_v2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(f"Loaded sample idx={idx} | class_id={class_id.item()} | image_tensor shape={tuple(image_tensor.shape)} | mask_tensor shape={tuple(mask_tensor.shape)} | bbox={bbox.squeeze(0).tolist()}")
...
def _collect_demo_indices_by_class(image_info: list, class_ids: list[int], limit_per_class: int = 1):
    """Collect up to `limit_per_class` indices for each requested class id."""
    remaining = {int(class_id): int(limit_per_class) for class_id in class_ids}
    demo_indices: dict[int, list[int]] = {int(class_id): [] for class_id in class_ids}

    for idx, sample_info in enumerate(image_info):
        class_id = int(sample_info["class_id"])
        if class_id not in remaining or remaining[class_id] <= 0:
            continue

        demo_indices[class_id].append(idx)
...
def _save_augmented_visualization(image_np, mask_np, bbox, title_left: str, title_right: str, out_path: str):
    import numpy as np
    ...
    ax = axes[1]
    ax.imshow(image_np)
    ax.imshow(mask_np, cmap="Reds", alpha=0.35 * mask_np, vmin=0.0, vmax=1.0)
    ax.add_patch(
        Rectangle(
            (x1, y1),
            max(0.0, x2 - x1),
...
def visualize_augmented_samples(
    ...
    demo_class_ids: Optional[list[int]] = None,
    demo_per_class: int = 1,
):
    ...
    demo_indices_by_class = _collect_demo_indices_by_class(
        dataset.image_info,
        class_ids=demo_class_ids,
        limit_per_class=demo_per_class,
    )

    render_jobs: list[tuple[int, str]] = [(idx, f"aug_sample_{n:02d}_idx_{idx}.png") for n, idx in enumerate(indices)]
    for class_id in demo_class_ids:
        class_name = CLASS_ID_TO_NAME.get(int(class_id), f"class_{class_id}")
        class_demo_indices = demo_indices_by_class.get(int(class_id), [])
        for demo_n, idx in enumerate(class_demo_indices):
            render_jobs.append((idx, f"demo_{class_name}_{demo_n:02d}_idx_{idx}.png"))

    for idx, filename in render_jobs:
        ...
        _save_augmented_visualization(
            image_np=image_np,
            mask_np=mask_np,
            bbox=bbox,
            title_left=f"Aug image + bbox | idx={idx} | class={class_id_int} ({class_name})",
            title_right=f"Aug image + mask + bbox | class={class_name}",
            out_path=out_path,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Extra demo visualizations to save for each of the like/dislike classes",
    )
