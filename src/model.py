...
class MBConv(nn.Module):
    """MobileNetV2-style Inverted Residual Block with Depthwise Convolutions and SE."""
    def __init__(self, c1: int, c2: int, stride: int = 1, expand_ratio: float = 2.0):
        super().__init__()
        self.stride = stride
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(Conv(c1, hidden_dim, k=1))
            
        layers.extend([
            Conv(hidden_dim, hidden_dim, k=3, s=stride, g=hidden_dim), # Depthwise
            SEBlock(hidden_dim),                                       # Attention
            nn.Conv2d(hidden_dim, c2, 1, 1, 0, bias=False),            # Pointwise linear
            nn.BatchNorm2d(c2)
        ])
...
class SegHead(nn.Module):
    ...
    def forward(self, p2: torch.Tensor, neck_features: list[torch.Tensor], output_size: tuple[int, int]) -> torch.Tensor:
...
class RGB_V1_BackBone(nn.Module):
    def __init__(self):
        ...
        self.stage1 = nn.Sequential(Conv(3, c1, k=3, s=2), Conv(c1, c2, k=3, s=2))       # P2
        self.stage2 = nn.Sequential(Conv(c2, c3, k=3, s=2), Conv(c3, c3, k=3, s=1))      # P3
        self.stage3 = nn.Sequential(Conv(c3, c4, k=3, s=2), Conv(c4, c4, k=3, s=1))      # P4
        self.stage4 = nn.Sequential(Conv(c4, c5, k=3, s=2), Conv(c5, c5, k=3, s=1))      # P5
...
class RGB_V1_Neck(nn.Module):
    def __init__(self, in_channels: dict[str, int]):
        ...
        # Simple Top-Down FPN
        self.lat5 = Conv(p5_ch, self.c5, 1)
        self.lat4 = Conv(p4_ch, self.c4, 1)
        self.lat3 = Conv(p3_ch, self.c3, 1)
        self.reduce5 = Conv(self.c5, self.c4, 1)  # 256 -> 128 before merging with p4
        self.reduce4 = Conv(self.c4, self.c3, 1)  # 128 -> 64  before merging with p3

        self.up = nn.Upsample(scale_factor=2, mode="nearest")
        self.smooth4 = Conv(self.c4, self.c4, 3)
        self.smooth3 = Conv(self.c3, self.c3, 3)

    def forward(self, p3: torch.Tensor, p4: torch.Tensor, p5: torch.Tensor):
        n5 = self.lat5(p5)
        n4 = self.smooth4(self.lat4(p4) + self.up(self.reduce5(n5)))
        n3 = self.smooth3(self.lat3(p3) + self.up(self.reduce4(n4)))
        return n3, n4, n5
...
class RGB_V1(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {"det": self.detect([n3, n4, n5]), "cls": self.cls_head(n5), "seg": self.seg_head(p2, [n3, n4, n5], x.shape[-2:])}
...
class RGB_V2_BackBone(nn.Module):
    def __init__(self):
        super().__init__()
        c1, c2, c3, c4, c5 = 16, 32, 64, 128, 256
        self.stage1 = nn.Sequential(Conv(3, c1, k=3, s=2), Conv(c1, c2, k=3, s=2))
        # Swap standard convs for MBConvs (Depthwise + SE)
        self.stage2 = nn.Sequential(Conv(c2, c3, k=3, s=2), MBConv(c3, c3, expand_ratio=2.0))
        self.stage3 = nn.Sequential(Conv(c3, c4, k=3, s=2), MBConv(c4, c4, expand_ratio=2.0), MBConv(c4, c4, expand_ratio=2.0))
        self.stage4 = nn.Sequential(Conv(c4, c5, k=3, s=2), MBConv(c5, c5, expand_ratio=2.0))
        self.channels = {"p2": c2, "p3": c3, "p4": c4, "p5": c5}
...
class RGB_V2_Neck(nn.Module):
    """PANet style Neck (Top-Down + Bottom-Up) for better localization and classification."""
    def __init__(self, in_channels: dict[str, int]):
        super().__init__()
        p3_ch, p4_ch, p5_ch = in_channels["p3"], in_channels["p4"], in_channels["p5"]
        self.c3, self.c4, self.c5 = 64, 128, 256
        ...
        self.reduce5 = Conv(self.c5, self.c4, 1)  # 256 -> 128 before merging with p4
        self.reduce4 = Conv(self.c4, self.c3, 1)  # 128 -> 64  before merging with p3
        self.td4 = MBConv(self.c4, self.c4, expand_ratio=1.0)
        self.td3 = MBConv(self.c3, self.c3, expand_ratio=1.0)

        # Bottom-up
        self.down3 = Conv(self.c3, self.c4, k=3, s=2)  # 64  -> 128
        self.down4 = Conv(self.c4, self.c5, k=3, s=2)  # 128 -> 256
        self.bu4 = MBConv(self.c4, self.c4, expand_ratio=1.0)
        self.bu5 = MBConv(self.c5, self.c5, expand_ratio=1.0)

    def forward(self, p3: torch.Tensor, p4: torch.Tensor, p5: torch.Tensor):
        # Top-down FPN
        t5 = self.lat5(p5)
        t4 = self.td4(self.lat4(p4) + self.up(self.reduce5(t5)))
        t3 = self.td3(self.lat3(p3) + self.up(self.reduce4(t4)))

        # Bottom-up PANet
        n3 = t3
        n4 = self.bu4(t4 + self.down3(n3))
        n5 = self.bu5(t5 + self.down4(n4))
...
class RGB_V3_Neck(nn.Module):
    def __init__(self, in_channels: dict[str, int]):
        ...
        self.n1_fpn = C3k2(p4_ch + p5_ch, self.c3, n=1, c3k=True, e=0.5, shortcut=True)
        ...
        self.n2_fpn = C3k2(p3_ch + p5_ch, self.c4, n=1, c3k=True, e=0.5, shortcut=True)
        ...
        self.n3_fpn = C3k2(p3_ch + p4_ch, self.c5, n=1, c3k=True, e=0.5, shortcut=True)
    ...
    def forward(self, p3: torch.Tensor, p4: torch.Tensor, p5: torch.Tensor):
        ...
        n1 = self.n1_fpn(torch.cat([self.up2(p4), self.up4(p5)], dim=1))
        ...
        n2 = self.n2_fpn(torch.cat([self.down_1(p3), self.up2(p5)], dim=1))
        ...
        n3 = self.n3_fpn(torch.cat([self.down_2(p3), self.down_3(p4)], dim=1))
...
class RGB_V4_BackBone(nn.Module):
    def __init__(self):
        super().__init__()
        # Nano scale channel dimensions
        c1, c2, c3, c4, c5 = 16, 32, 64, 128, 256
...
class RGB_V4(nn.Module):
    def __init__(self, num_classes: int, reg_max: int = 1):
        super().__init__()
        self.backbone = RGB_V4_BackBone()
        self.neck = RGB_V4_Neck(in_channels=self.backbone.channels)
        neck_ch = (self.neck.c3, self.neck.c4, self.neck.c5)
        self.detect = DetectionHead(nc=num_classes, ch=neck_ch, reg_max=reg_max)
        self.cls_head = ClsHead(nc=num_classes, in_ch=neck_ch[2])  # Use the deepest neck feature for classification
        self.seg_head = SegHead(p2_ch=self.backbone.channels["p2"], neck_ch=neck_ch)
...
if __name__ == "__main__":
    ...
    print("Testing RGB_V1...")
    model_v1 = RGB_V1(10)
    torchinfo.summary(model_v1, input_data=x)
    out_v1 = model_v1(x)
    print(f"V1 Det Box: {out_v1['det']['boxes'].shape}, Cls: {out_v1['cls'].shape}, Seg: {out_v1['seg'].shape}")

    print("Testing RGB_V2...")
    model_v2 = RGB_V2(10)
    torchinfo.summary(model_v2, input_data=x)
    out_v2 = model_v2(x)
    print(f"V2 Det Box: {out_v2['det']['boxes'].shape}, Cls: {out_v2['cls'].shape}, Seg: {out_v2['seg'].shape}")


    print("Testing RGB_V3...")
    model_v3 = RGB_V3(10)
    torchinfo.summary(model_v3, input_data=x)
    out_v3 = model_v3(x)
    print(f"V3 Det Box: {out_v3['det']['boxes'].shape}, Cls: {out_v3['cls'].shape}, Seg: {out_v3['seg'].shape}")

    print("Testing RGB_V4...")
    model_v4 = RGB_V4(10)
    torchinfo.summary(model_v4, input_data=x)
    out_v4 = model_v4(x)
    print(f"V4 Det Box: {out_v4['det']['boxes'].shape}, Cls: {out_v4['cls'].shape}, Seg: {out_v4['seg'].shape}")