...
class GestureEncoder(nn.Module):
    def __init__(self, in_channels: int = 3, drop_path_rate: float = 0.08) -> None:
        ...
        self.out_channels = {"c1": 32, "c2": 64, "c3": 128, "c4": 192, "c5": 320}
...
class SegmentationDecoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        shallow_dim = decoder_channels // 2
        ...
        self.project1 = ConvBNAct(shallow_channels, shallow_dim, kernel_size=1)
        ...
        self.decode1 = FusionBlock(decoder_channels + shallow_dim, decoder_channels)
        self.refine = nn.Sequential(
    ...
    def forward(
        ...
        d1 = self.decode1(p1, F.interpolate(d2, size=p1.shape[-2:], mode="bilinear", align_corners=False))

        detail_feat = torch.abs(d1 - F.avg_pool2d(d1, kernel_size=3, stride=1, padding=1))
        decoder_feat = self.refine(torch.cat([d1, detail_feat], dim=1))
...
def local_detail_map(x: torch.Tensor) -> torch.Tensor:
    return torch.abs(x - F.avg_pool2d(x, kernel_size=3, stride=1, padding=1))
...
class MultiScaleClassificationHead(nn.Module):
    ...
    def forward(
        ...
        att_logits = self.local_attention(fused) + F.interpolate(
            attention_logits,
            size=target_size,
            mode="bilinear",
            align_corners=False,
        ...
        pooled_detail = weighted_average_pool(local_detail_map(fused), weights)
        ...
        shape = spatial_moments(weights)
        gate_strength = torch.sigmoid(att_logits).mean(dim=(2, 3))

        embedding = torch.cat(
            [pooled_focus, pooled_detail, pooled_avg + pooled_max, context, shape, gate_strength],
            dim=1,
        )
...
class HandGestureMultiTask(nn.Module):
    def __init__(self, num_classes: int = 10, seg_out_channels: int = 1) -> None:
        ...
        self.segmentation_head = SegmentationDecoder(
            channels=160,
            shallow_channels=self.backbone.out_channels["c1"],
    ...
    def forward(self, x: torch.Tensor, tasks: tuple[str, ...] | None = None) -> dict[str, object]:
        ...
        seg_logits, decoder_feat, attention_logits = self.segmentation_head(
            pyramid,
            shallow_feat=features["c1"],
