...
class PureConvBlock(nn.Module):
    def __init__(self, c1: int, c2: int, n: int = 2):
        super().__init__()
        layers = [Conv(c1, c2, k=3, s=1)]
        # Stack subsequent convolutions to match the depth/parameters of a Bottleneck
        for _ in range(n - 1):
            layers.append(Conv(c2, c2, k=3, s=1))
...
class RGB_V1_BackBone(nn.Module):
    """Purely Sequential Convolutional Backbone (Baseline)"""
    def __init__(self):
        super().__init__()
        ...
        self.stage1 = nn.Sequential(
            Conv(3, c1, k=3, s=2),
            Conv(c1, c2, k=3, s=2),
            PureConvBlock(c2, c3, n=2)  # Replaces C3k2
        )

        # Stage 2: Downsamples by 8 (Yields P3)
        self.stage2 = nn.Sequential(
            Conv(c3, c3, k=3, s=2),
            PureConvBlock(c3, c4, n=2)  # Replaces C3k2
        )

        # Stage 3: Downsamples by 16 (Yields P4)
        self.stage3 = nn.Sequential(
            Conv(c4, c4, k=3, s=2),
            PureConvBlock(c4, c4, n=3)  # n=3 roughly approximates C3k2 with c3k=True
        )

        # Stage 4: Downsamples by 32 (Yields P5)
        self.stage4 = nn.Sequential(
            Conv(c4, c5, k=3, s=2),
            PureConvBlock(c5, c5, n=3),
            # Replaces multi-scale SPPF with a naive, single-scale MaxPool sequence
...
class RGB_V1_Neck(nn.Module):
    """Purely Sequential FPN Neck (Baseline)"""
    def __init__(self, in_channels: dict[str, int]):
        ...
        # Use PureConvBlock instead of C3k2 for feature fusion
        self.n1_fpn = PureConvBlock(p4_ch + p5_ch, self.c3, n=2)
        self.n2_fpn = PureConvBlock(p3_ch + p5_ch, self.c4, n=2)
        self.n3_fpn = PureConvBlock(p3_ch + p4_ch, self.c5, n=2)
...
class RGB_V1(nn.Module):
    """Baseline Multi-Task Model (Pure Convolutions)"""
