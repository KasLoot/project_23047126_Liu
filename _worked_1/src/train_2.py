...
@dataclass
class TrainConfig:
    dataset_path: str = "/home/yuxin/Object_Detection/project_23047126_Liu/dataset/processed_full_dataset"
    output_dir: str = "/home/yuxin/Object_Detection/project_23047126_Liu/outputs/yolo26_multi"
    epochs: int = 10
    batch_size: int = 32
    num_workers: int = 0
    lr: float = 5e-4
...
def build_dataloaders(cfg: TrainConfig) -> tuple[DataLoader, DataLoader]:
    ...
    train_dataset = HandGestureDataset(dataset_path=cfg.dataset_path, transform=train_aug)
    val_dataset = HandGestureDataset(dataset_path=cfg.dataset_path, transform=None)
    pin_memory = torch.cuda.is_available()

    n_total = len(train_dataset)
    val_len = int(n_total * cfg.val_split)
    ...
    train_loader = DataLoader(train_subset, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.num_workers, pin_memory=pin_memory)
    val_loader = DataLoader(val_subset, batch_size=cfg.batch_size, shuffle=False, num_workers=cfg.num_workers, pin_memory=pin_memory)
...
def build_model_and_profile_gpu_usage(device: torch.device) -> YOLO26MultiTask:
    ...
    if device.type != "cuda":
        ...
        model = YOLO26MultiTask(yolo_weights_path="outputs/yolo26_scratch/best.pt", num_classes=10).to(device)
    ...
    dummy_image = torch.randn(1, 3, 480, 640, device=device)
    model.eval()
    with torch.no_grad():
        _ = model(dummy_image)
    torch.cuda.synchronize(device)
...
def train() -> None:
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        torch.save(model.state_dict(), f"{cfg.output_dir}/YOLO26MultiTask_last.pt")
        ...
        with torch.no_grad():
            ...
            if epoch_val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), f"{cfg.output_dir}/YOLO26MultiTask_best.pt")

        # Clean, easy-to-read logging
        print(f"Epoch {epoch:02d}/{cfg.epochs} | "
              f"Loss (Train: {epoch_train_loss/len(train_loader):.3f} | Val: {epoch_val_loss/len(val_loader):.3f}) || "
